...
void lectura_tensionAC() {
  ...
  float lectura, valor_inst = 0, value, ki = 0.1470588 /*5/34*/, kv = 68.86270491 /* 55 * 611 / (8 * 51)*/;
  ...
  if (rele[contador].On == true || contador == Equipos) {
    ...
    if (contador < Equipos) {                                                   //Sincronismo mediante la variable Equipos, está me define que dato envio a labview.
      rele[contador].medicion = analogRead(A0 + contador) * (5.0 / 1023.0);     //value * 5 * voltage * ki / 1023;
      ...
      if (rele[0].On == false && rele[1].On == true && rele[2].On == true && rele[3].On == true) {
        Serial.print("V=");
